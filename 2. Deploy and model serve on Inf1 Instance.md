...
# 2. Deploy and Serve Compiled Model On Inferentia Instance (30 Minutes)


1.1. Select Deep Learning AMI Version 2.6 for Ubuntu, and start an Inf1 instance inf1.2xlarge.


### Update Repo List for Pip and Apt-get (Debian)

2.1 Add Neuron Apt-get repo for debian, Ubuntu 16.
...
2.3 Installing Neuron run-time, SDK, and Tensorflow Model server optimized for the Neuron SDK.
...
2.4. Add Neuron Pip Repo to system for easy update.
...
2.6. Setup a new Python 3.6 environment:
...
2.7. Install TensorFlow-Neuron (Neuron Compiler is not needed if compilation is not done)
```bash
pip install tensorflow-neuron
```

2.8. Also, you would need TensorFlow Serving API (use --no-deps to prevent installation of regular tensorflow):

```bash
pip install --no-deps tensorflow_serving_api==1.15
```

2.9. Install Pillow (graphics package for loading images)
```bash
pip install pillow
```
...
Steps Overview:
1. You already compiled the Keras ResNet50 model and export it as a SavedModel which is an interchange format for TensorFlow models. ssh copy (scp) the compiled package onto the Inferentia Instance.
2. Run inference on Inf1 with an example input.


3.1. If not compiling and inferring on the same instance, copy the artifact to the inference server:
...
```
3.2. On the Inf1, create a inference Python script named `infer_resnet50.py` with the following content:
```python
import os
import time
import numpy as np
...
3.3. Unzip the mode on Inferentia Instance, download the example image and run the inference:
```bash
unzip resnet50_neuron.zip
```
3.4. Get a sample image to test the model inference on Neuron Cores.
```bash
curl -O https://raw.githubusercontent.com/awslabs/mxnet-model-server/master/docs/images/kitten_small.jpg
```
3.5. Run Inference Script
```bash
...
### 4.1. Prepare Compiled Saved Model
...
### 4.3. Generate inference requests to the model server
